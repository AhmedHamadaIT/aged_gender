...
def capture_preview_frame(
    ...
    *,
    ...
    """
    Grab one BGR frame using the same ingest path as ``frames()`` (GStreamer /
    adaptive FFmpeg / OpenCV). Discards the first ``settle_after_reads - 1`` yielded
    frames, then returns a copy of the next, or ``None`` if not enough frames arrive.
    """
    ...
    gen = frames(source, camera_id=camera_id)
    try:
        chosen = None
